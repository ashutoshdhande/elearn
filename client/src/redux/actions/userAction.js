...
export const addCourse = (courseCredentials, history) => {
  return async (dispatch) => {
    try {
      ...
      const { data } = await axios({
        method: "Post",
        url: "http://localhost:5000/api/user/addCourse",
...
export const allCourses = () => {
  return async (dispatch) => {
    try {
      const { data } = await axios({
        method: "Get",
        url: "http://localhost:5000/api/user/getAllCourse",
      });
      dispatch({
...
export const addToCart = (courseId) => {
  return async (dispatch) => {
    try {
      const { data } = await axios({
        method: "Get",
        url: `http://localhost:5000/api/user/addToCart/${courseId}`,
      });
...
export const buyCourse = (courseId, history) => {
  return async (dispatch) => {
    try {
      const { data } = await axios({
        method: "Get",
        url: `http://localhost:5000/api/user/buyCourse/${courseId}`,
      });
      dispatch(userLoginHelper(data.message));
...
export const myCourses = () => {
  return async (dispatch) => {
    try {
      const { data } = await axios({
        method: "Get",
        url: `http://localhost:5000/api/user/userCourses`,
      });
      const { message } = data;
...
export const courseDetailsById = (courseId) => {
  return async (dispatch) => {
    try {
      const { data } = await axios({
        method: "Get",
        url: `http://localhost:5000/api/user/getCourseById/${courseId}`,
      });
      const { message } = data;
      dispatch({
...
export const askQuestion = (courseId, question) => {
  return async (dispatch) => {
    try {
      const { data } = await axios({
        method: "Post",
        url: `http://localhost:5000/api/user/commentOnCourse/${courseId}`,
        data: { question },
      });
      const { message } = data;
      dispatch({
